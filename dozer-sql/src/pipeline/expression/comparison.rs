            ...
            match left_p {
                ...
                Field::Int(left_v) => match right_p {
                    ...
                    Field::UInt(right_v) => Ok(Field::Boolean($function(
                        ...
                        right_v
                            .to_i64()
                            .ok_or(PipelineError::UnableToCast(format!("{}", right_v), "i64"))?,
                    ))),
                    ...
                    Field::Decimal(right_v) => {
                        let left_v_d =
                            Decimal::from_i64(left_v).ok_or(
                                PipelineError::UnableToCast(format!("{}", left_v), "Decimal"))?;
                        Ok(Field::Boolean($function(left_v_d, right_v)))
                    }
                    // left: Int, right: Null
                    Field::Null => Ok(Field::Boolean(false)),
                    _ => Err(PipelineError::InvalidTypeComparison(left_p, right_p, $op.to_string())),
                },
                Field::UInt(left_v) => match right_p {
                    ...
                    Field::Int(right_v) => Ok(Field::Boolean($function(
                        left_v
                            .to_i64()
                            .ok_or(PipelineError::UnableToCast(format!("{}", left_v), "i64"))?,
                        right_v,
                    ))),
                    ...
                    Field::Float(right_v) => {
                        let left_v_f = OrderedFloat::<f64>::from_i64(left_v.to_i64().ok_or(
                            PipelineError::UnableToCast(format!("{}", left_v), "i64"))?)
                        .unwrap();
                        Ok(Field::Boolean($function(left_v_f, right_v)))
                    }
                    ...
                    Field::Decimal(right_v) => {
                        let left_v_d = Decimal::from_i64(left_v.to_i64().ok_or(
                            PipelineError::UnableToCast(format!("{}", left_v), "i64"))?)
                        .ok_or(PipelineError::UnableToCast(format!("{}", left_v), "Decimal"))?;
                        Ok(Field::Boolean($function(left_v_d, right_v)))
                    }
                ...
                Field::Float(left_v) => match right_p {
                    ...
                    Field::UInt(right_v) => {
                        let right_v_f = OrderedFloat::<f64>::from_i64(right_v.to_i64().ok_or(
                            PipelineError::UnableToCast(format!("{}", right_v), "Decimal"))?)
                        .unwrap();
                ...
                Field::Decimal(left_v) => match right_p {
                    ...
                    Field::Float(right_v) => {
                        let right_v_d = Decimal::from_f64(right_v.to_f64().ok_or(
                            PipelineError::UnableToCast(format!("{}", right_v), "f64"))?)
                        .ok_or(
                            PipelineError::UnableToCast(format!("{}", right_v), "Decimal"))?;
                    ...
                    Field::UInt(right_v) => {
                        let right_v_d = Decimal::from_i64(right_v.to_i64().ok_or(
                            PipelineError::UnableToCast(format!("{}", right_v), "i64"))?)
                        .ok_or(
                            PipelineError::UnableToCast(format!("{}", right_v), "Decimal"))?;
                        Ok(Field::Boolean($function(left_v, right_v_d)))
                    }
                ...
                Field::Binary(_left_v) => Err(PipelineError::InvalidTypeComparison(left_p, right_p, $op.to_string())),
                _ => Err(PipelineError::InvalidTypeComparison(left_p, right_p, $op.to_string())),
            }
        }
    };
...
pub fn evaluate_lt(
    ...
    match left_p {
        ...
        Field::Int(left_v) => match right_p {
            ...
            Field::Null => Ok(Field::Boolean(false)),
            _ => Err(PipelineError::InvalidTypeComparison(left_p, right_p, "<".to_string())),
        },
        Field::Float(left_v) => match right_p {
        ...
        Field::String(left_v) => match right_p {
            ...
            Field::Null => Ok(Field::Boolean(false)),
            _ => Err(PipelineError::InvalidTypeComparison(left_p.clone(), right_p, "<".to_string())),
        },
        Field::Timestamp(left_v) => match right_p {
        ...
        Field::Binary(_left_v) => Err(PipelineError::InvalidTypeComparison(left_p.clone(), right_p, "<".to_string())),
        _ => Err(PipelineError::InvalidTypeComparison(left_p, right_p, "<".to_string())),
    }
}
