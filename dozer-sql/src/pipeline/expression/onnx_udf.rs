...
pub fn evaluate_onnx_udf(
    ...
    match (field.clone(), return_type) {
        (Field::String(v), FieldType::String | FieldType::Json | FieldType::Float) => {
            let array = ndarray::CowArray::from(Array::from_shape_vec((1,), vec![v]).unwrap().into_dyn());
            ...
            match return_type {
                ...
                FieldType::Float => {
                    let output_array_view = output.try_extract::<f32>().unwrap();
                    Ok(Field::Float(OrderedFloat(output_array_view.view().deref()[0].clone().into())))
                }
        ...
        (Field::UInt(v), FieldType::Json | FieldType::Float | FieldType::UInt) => {
            ...
            match return_type {
                ...
                FieldType::UInt => {
                    ...
                    Ok(Field::UInt(output_array_view.view().deref()[0].clone().into()))
                }
                _ => Err(InvalidValue(format!("{field} incompatible with {return_type}")))
            }
        },
        (Field::U128(v), FieldType::Json | FieldType::Float | FieldType::U128) => {
            ...
            let array = ndarray::CowArray::from(Array::from_shape_vec((1,), vec![u64::try_from(v)
                .map_err(|e| InvalidType(field.clone(), return_type.to_string()))?]).unwrap()
                .into_dyn());
            let input_tensor_values = vec![Value::from_array(session.allocator(), &array).unwrap()];
            let outputs: Vec<Value> = session.run(input_tensor_values).unwrap();
            let output = outputs[0].borrow();
            ...
            match return_type {
                ...
                FieldType::U128 => {
                    let output_array_view = output.try_extract::<u32>().unwrap();
                    Ok(Field::U128(output_array_view.view().deref()[0].clone().into()))
                }
                _ => Err(InvalidValue(format!("{field} incompatible with {return_type}")))
            }
        },
        (Field::Int(v), FieldType::Json | FieldType::Float | FieldType::Int) => {
            ...
            match return_type {
                ...
                FieldType::Int => {
                    ...
                    Ok(Field::Int(output_array_view.view().deref()[0].clone().into()))
                }
                _ => Err(InvalidValue(format!("{field} incompatible with {return_type}")))
            }
        },
        (Field::I128(v), FieldType::Json | FieldType::Float | FieldType::I128) => {
            warn!("Precision loss happens due to conversion");
            let array = ndarray::CowArray::from(Array::from_shape_vec((1,), vec![i64::try_from(v)
                .map_err(|e| InvalidType(field.clone(), return_type.to_string()))?]).unwrap()
                .into_dyn());
            let input_tensor_values = vec![Value::from_array(session.allocator(), &array).unwrap()];
            let outputs: Vec<Value> = session.run(input_tensor_values).unwrap();
            ...
            match return_type {
                ...
                FieldType::I128 => {
                    let output_array_view = output.try_extract::<i32>().unwrap();
                    Ok(Field::I128(output_array_view.view().deref()[0].clone().into()))
                }
                _ => Err(InvalidValue(format!("{field} incompatible with {return_type}")))
            }
        },
        (Field::Float(v), FieldType::Json | FieldType::Float) => {
            ...
            let array = ndarray::CowArray::from(Array::from_shape_vec((1,), vec![num]).unwrap().into_dyn());
            let input_tensor_values = vec![Value::from_array(session.allocator(), &array).unwrap()];
            let outputs: Vec<Value> = session.run(input_tensor_values).unwrap();
            let output = outputs[0].borrow();
