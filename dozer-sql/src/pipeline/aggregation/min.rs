...
impl MinAggregator {
    ...
    pub(crate) fn insert(
        ...
        match *new {
            Int(_i) => {
                ...
                ptx.put(&db, curr.to_ne_bytes().as_slice(), curr_count.to_ne_bytes().as_slice()).unwrap();
    ...
    pub(crate) fn update(
        ...
        let prev = match Self::delete(curr_state, old, curr_count, ptx, db).unwrap().first() {
            Some(v) => i64::from(*v),
            None => 0_i64,
        };
