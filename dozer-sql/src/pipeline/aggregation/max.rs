...
impl MaxAggregator {
        ...
        match *new {
            ...
            Timestamp(_t) => {
                ...
                if maximum == min_datetime {
                    ...
                    Ok(AggregationResult::new(
                        Self::get_value(
                            maximum.timestamp_millis().to_le_bytes().as_slice(),
                            return_type,
                        ),
                        Some(Vec::from(maximum.timestamp_millis().to_le_bytes())),
                    ))
                }
            }
            Date(_d) => {
    ...
    fn calc_f64_max(
        ...
        if ptx_cur.last()? {
            ...
            maximum = f64::from_le_bytes(deserialize!(cur.0));
        }
        Ok(maximum)
    }
    ...
    fn calc_timestamp_max(
        ...
        if ptx_cur.last()? {
            let cur = try_unwrap!(ptx_cur.read()).unwrap();
            maximum = Utc.timestamp_millis(i64::from_le_bytes(deserialize!(cur.0)));
        }
    ...
    fn calc_i64_max(
        ...
        if ptx_cur.last()? {
            let cur = try_unwrap!(ptx_cur.read()).unwrap();
            maximum = i64::from_le_bytes(deserialize!(cur.0));
        }
        Ok(maximum)
    }
}
