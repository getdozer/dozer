...
use dozer_shared::types::{Field, Operation, OperationEvent};
...
impl SelectionProcessor {
    pub fn new(id: i32, input_ports: Option<Vec<PortHandle>>, output_ports: Option<Vec<PortHandle>>, operator: Box<dyn Expression>) -> Self {
        Self { id, input_ports, output_ports, operator}
    }
}
...
impl Processor for SelectionProcessor {
    ...
    fn process(&self, from_port: Option<PortHandle>, op: OperationEvent, ctx: & dyn ExecutionContext, fw: &ChannelForwarder) -> Result<NextStep, String> {
        ...
        match op.operation {
            ...
            Operation::Update {table_name, old, new} => {
                Err("UPDATE Operation not supported.".to_string())
            },
            Operation::Terminate => {
                Err("TERMINATE Operation not supported.".to_string())
            },
            _ => {
                Err("TERMINATE Operation not supported.".to_string())
            },
