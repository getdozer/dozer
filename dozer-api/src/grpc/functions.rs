use crate::common_query::{get_record, get_records};
...
pub async fn grpc_list(
    ...
    let result = get_records(cache, exp).unwrap();
...
pub async fn grpc_get_by_id(
    ...
    println!("==== id_input {}", id_input);
    let result = get_record(cache, id_input.to_owned())
