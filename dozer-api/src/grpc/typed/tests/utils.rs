use std::{env, path::PathBuf};

use crate::test_utils::{self, get_sample_records};

use super::super::{helper::query_response_to_typed_response, utils::get_proto_descriptor};

#[test]
fn test_records_to_typed_response() {
    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());
    let path = out_dir
        .join("generated_films.bin")
        .to_string_lossy()
        .to_string();

    let desc = get_proto_descriptor(path).unwrap();

    let (schema, _) = test_utils::get_schema();
    let endpoint_name = "films".to_string();

    let records = get_sample_records(schema.clone());
    let res = query_response_to_typed_response(records, schema, desc, endpoint_name);
    let film = res.message.get_field_by_name("films");
    assert!(film.is_some(), "films must be present");
}
