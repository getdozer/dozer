...
impl<T> fmt::Debug for ValidationError<T> where
T: Serialize + std::fmt::Debug {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ValidationError").field("details", &self.details).field("message", &self.message).finish()
    }
}


impl<T> fmt::Display for ValidationError<T> where
T: Serialize {
...
impl<T> error::ResponseError for ValidationError<T> where
T: Serialize + std::fmt::Debug {
