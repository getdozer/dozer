    ...
    match schema.name.clone() {
        None => match schema.r#type.clone() {
            Value::String(typ) => match typ.as_str() {
                ...
                _ => Err(KafkaSchemaError::TypeNotSupported(typ)),
            },
            _ => Err(KafkaSchemaError::TypeNotSupported(
                "Unexpected value type".to_string(),
            )),
        },
...
pub fn map_schema(
    ...
    match &schema.fields {
        None => Err(KafkaSchemaError::SchemaDefinitionNotFound),
        Some(fields) => {
            ...
            if let Some(schema) = new_schema_struct {
                ...
                Err(KafkaSchemaError::SchemaDefinitionNotFound)
            }
        }
    }
}
...
mod tests {
    ...
    fn test_it_fails_when_schema_empty() {
        ...
        assert_eq!(actual_error, KafkaSchemaError::SchemaDefinitionNotFound);
    }
    ...
    fn test_map_type() {
        ...
        test_map_type!(
            "not found",
            None,
            Err(KafkaSchemaError::TypeNotSupported("not found".to_string()))
        );
        test_map_type!(
            "int8",
        ...
        test_map_type!(
            ...
            Err(KafkaSchemaError::TypeNotSupported(
