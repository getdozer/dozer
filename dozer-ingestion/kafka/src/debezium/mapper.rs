            ...
            "bytes" => value.as_str().map_or(Ok(Field::Null), |s| {
                Ok(Field::Binary(
                    engine::general_purpose::STANDARD
                        .decode(s)
                        .map_err(KafkaSchemaError::BinaryDecodeError)?,
                ))
            }),
        ...
        .map(|f| match value.get(f.name.clone()).cloned() {
            ...
            Some(field_value) => {
                let schema_struct = fields_map
                    .get(&f.name)
                    .ok_or_else(|| KafkaSchemaError::FieldNotFound(f.name.clone()))?;
...
mod tests {
    use dozer_ingestion_connector::dozer_types::{
        ...
        serde_json::Map,
        types::{FieldDefinition, FieldType, SourceDefinition},
    };
    ...
    fn it_converts_value_to_field_error() {
        test_conversion_debezium_error!(
            ...
            "Unknown type",
            None,
            KafkaSchemaError::TypeNotSupported("Unknown type".to_string()),
            None
        );
        // test_conversion_debezium_error!(
        //     1234,
