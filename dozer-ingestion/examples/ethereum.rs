...
fn main() {
    ...
    let wss_url = std::env::var("ETH_WSS_URL")
        .unwrap_or_else(|_| panic!(r#"ETH_WSS_URL environment variable is not set. "#));

    info!("Initializing with WSS: {}", wss_url);

    let (ingestor, iterator) = Ingestor::initialize_channel(IngestionConfig::default());

    let producer = || -> Result<(), ConnectorError> {
        let address = "0xb47e3cd837ddf8e4c57f05d70ab865de6e193bbb".to_string();
        let mut eth_connector = EthConnector::new(
            ...
            EthConfig {
                filter: Some(EthFilter {
                    from_block: Some(16258680),
                    to_block: Some(16227379),
