...
impl DeltaLakeReader {
    ...
    pub fn read(
        &self, table: &[TableInfo], ingestor: &Ingestor
    ) -> ConnectorResult<()> {
        for (id, table) in table.iter().enumerate() {
            Runtime::new().unwrap().block_on(self.read_impl(id as u32, table, ingestor))?;
        }
        Ok(())
    }

    async fn read_impl(&self, id: u32, table: &TableInfo, ingestor: &Ingestor) -> ConnectorResult<()> {
        ...
        let data = ctx.read_table(Arc::new(delta_table))?
            .select_columns(&cols)?.execute_stream().await?;
        ...
        while let Some(Ok(batch)) = data.next().await {
            for row in 0..batch.num_rows() {
                let fields = batch
                    ...
                    .enumerate()
                    .map(|(col, column)| {
                        map_value_to_dozer_field(column, &row, cols[col]).unwrap()
                    })
                    .collect::<Vec<_>>();
                ...
                ingestor
                    .handle_message((
                        ...
                        IngestionMessage::OperationEvent(Operation::Insert {
                            new: Record {
                                schema_id: Some(SchemaIdentifier { id: id as u32, version: 0 }),
                                values: fields,
                                version: None,
                            },
                        }),
    ...
    pub fn table_path(&self, table_name: &str) -> ConnectorResult<String> {
        ...
        Err(ConnectorError::TableNotFound(format!("Delta table: {table_name} can't find")))
    }
}