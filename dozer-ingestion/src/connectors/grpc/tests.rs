...
use dozer_types::{arrow::array::{Int32Array, StringArray}, grpc_types::{
    ingest::{ingest_service_client::IngestServiceClient, IngestArrowRequest, IngestRequest},
    types,
}, ingestion_types::IngestionMessageKind, models::connection::{Connection, ConnectionConfig}, serde_json, serde_json::Value, types::Operation};
use dozer_types::{
...
async fn ingest_grpc_arrow() {
    let schema_str = serde_json::to_string(&DozerSchema::empty()
        ...
        .clone()).expect("Schema can always be serialized as JSON");
    ...
    // Ingest a record
    let schema = arrow_types::Schema::new_with_metadata(vec![
        arrow_types::Field::new("id", arrow_types::DataType::Int32, false),
    ...
    let record_batch =
        RecordBatch::try_new(Arc::new(schema), vec![Arc::new(a), Arc::new(b), Arc::new(c)]).unwrap();
    ...
    if let IngestionMessageKind::OperationEvent(op) = msg.kind {
        if let Operation::Insert { new: record } = op {
            assert_eq!(record.values[0].as_int(), Some(1675));
            assert_eq!(record.values[1].as_string(), Some("dario"));
            assert_eq!(record.values[2].as_json(), Some(
                &dozer_JsonValue::Array(
                    vec![
