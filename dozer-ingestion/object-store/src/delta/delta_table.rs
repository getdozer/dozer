...
use dozer_ingestion_connector::{
    ...
    dozer_types::{
        ...
        chrono::{DateTime, Utc},
        log::error,
        models::ingestion_types::{DeltaConfig, IngestionMessage},
...
use crate::{adapters::DozerObjectStore, table_watcher::TableWatcher, ObjectStoreConnectorError};
...
impl<T: DozerObjectStore + Send> TableWatcher for DeltaTable<T> {
    ...
    > {
        ...
        let h = tokio::spawn(async move {
            ...
            while let Some(Ok(batch)) = data.next().await {
                let dozer_schema = map_schema_to_dozer(&batch.schema()).unwrap();
                for row in 0..batch.num_rows() {
                    let fields = batch
                        .columns()
                        .iter()
