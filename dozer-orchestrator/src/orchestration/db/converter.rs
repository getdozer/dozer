...
impl TryFrom<Connection> for DbConnection {
    ...
    fn try_from(item: Connection) -> Result<Self, Self::Error> {
        ...
        let json: Value = serde_json::to_value(&authentication)
...
impl TryFrom<DbConnection> for Connection {
    ...
    fn try_from(item: DbConnection) -> Result<Self, Self::Error> {
        ...
        println!("==== db connection: {:?}",authentication_str );
        let authentication = serde_json::from_str::<crate::orchestration::models::connection::Authentication>(&authentication_str).map_err(|err| string_to_static_str(err.to_string()))?;
        let db_type: DBType = DBType::from_str(&item.db_type).map_err(|err| string_to_static_str(err.to_string()))?;
