...
pub trait IterationForwarder: Send + Sync {
    fn forward(&self, event: IngestionOperation, schema_id: u16) -> Result<(), OrchestrationError>;
}

pub struct ChannelForwarder {
    pub sender: crossbeam::channel::Sender<(IngestionOperation, u16)>,
}
...
impl IterationForwarder for ChannelForwarder {
    fn forward(
        ...
        let send_res = self.sender.send((operation, schema_id));
        match send_res {
            Ok(_) => Ok(()),
        ...
        for connection in connections {
            ...
            spawn(move || -> Result<(), OrchestrationError> {
                ...
                loop {
                    ...
                    let schema_id = match msg.clone() {
                        IngestionOperation::OperationEvent(op) => match op.operation {
                            Operation::Delete { old } => old.schema_id,
                            Operation::Insert { new } => new.schema_id,
                            Operation::Update { old: _, new } => new.schema_id,
                        },
                        IngestionOperation::SchemaUpdate(schema) => schema.identifier,
                    }
                    .unwrap();
