...
use dozer_types::epoch::{Epoch, ExecutorOperation, RefOperation};
...
impl StateWriter {
    ...
    fn store_op(
        ...
        op: RefOperation,
        port: &PortHandle,
    ) -> Result<RefOperation, RecordWriterError> {
...
impl ChannelManager {
    #[inline]
    fn send_op(&mut self, mut op: RefOperation, port_id: PortHandle) -> Result<(), ExecutionError> {
        if self.stateful {
