...
"#;
    let deserializer_result = serde_yaml::from_str::<Config>(input_config_without_flag).unwrap();
    assert_eq!(deserializer_result.cache_max_map_size, None);
    assert_eq!(deserializer_result.app_max_map_size, None);
    assert_eq!(deserializer_result.commit_timeout, None);
    assert_eq!(deserializer_result.app_buffer_size, None);
    assert_eq!(deserializer_result.commit_size, None);
}
